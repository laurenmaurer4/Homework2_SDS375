...
The expected difference in fev1 between children whose ages happens to differ by one is 0.17 liters. 
...
ggplot(dataset, aes(x=age, y=fev1)) +
  geom_point() +
  labs(title = "Actual Plot")

# Hypothetical plot
x <- runif(200, min = 5, max = 18)  # Uniform distribution between 5 and 18
y <- x/4 + rnorm(200, mean = 0, sd = .5)
y_ <- pmin(pmax(y, 0.5), 5)  # clamp y between 0.5 and 5
sim_data <- data.frame(age = x, fev1 = y)

ggplot(sim_data, aes(x = age, y = fev1)) +
  geom_point() +
  labs(title = "Simulated Data", x = "Age", y = "FEV1")+
  labs(title = "Simulated Plot")
```
